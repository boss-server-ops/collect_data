...
class RobotClient:
    ...
    def stop(self):
        ...
        self.logger.debug("Robot disconnected") #调试用

        # If the robot is a ROS2 node, shutdown the executor and spin thread
        ...
        self.channel.close()
        self.logger.debug("Client stopped, channel closed") #调试用
    ...
    def _set_task(self, new_task: str):
        ...
        with self.action_queue_lock:
            ...
            self.logger.info("任务切换：初始化清空一次动作队列")

        # ==== 不同任务设定不同的参数 ====
        # 有时候vla需要几秒钟才能让机器人真正动起来，而不是抖动
        ...
        with self.latest_action_lock:
            self.latest_action = -1
        self.logger.debug(f"Reset latest_action to -1 for new task '{new_task}'")
        self.action_chunk_size = -1

        # ==== 任务切换防竞态保护 ====
        # 给策略服务器一些时间完成 reset（典型 0.1~0.3 秒）
        time.sleep(0.3)
        self.logger.debug("Task switch delay 0.3s to ensure server reset complete.")
